...
% Questo codice applica un metodo multistep a 3 passi di tipo Backward 
% Differentiation Formulae, il metodo ha ordine 3. 
% Il presente metodo numerico non è A-stabile (ovverosia 
% incondizionatamente assolutamente stabile) ma è solo theta-stabile con
% angolo theta di approssimativamente 86.03°. 
% La mancanza della A-stabilità di BDF_3 si spiega anche con il teorema 
% della Seconda Barriera di Dahlquist.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Per far partire il metodo mi servono i 3 nodi iniziali ricavati con 
    % un metodo numerico ad un passo di ordine almeno 3. 
    % Utilizzo a tal fine ad esempio il metodo di Radau_IIA_5 (L-stabile) 
    % di ordine 5
    % Oss: uno di questi 3 punti è y_0 ed è comunque già noto

    mesh_aux = [t_0, t_0+h, t_0+2*h];
    U_sol_iniz = Radau_IIA_5(M,y_0,mesh_aux);

    u_n_meno_3 = y_0;   % = U_sol_iniz(:,1)
    u_n_meno_2 = U_sol_iniz(:,2);
    u_n_meno_1 = U_sol_iniz(:,3);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %                  IDEA DI RISOLUZIONE: 
    % Metodo BDF3:
    % u_{n} = 18/11*u_{n-1} - 9/11*u_{n-2} + 2/11*u_{n-3} + 6/11*h*f_{n}
    % dove con f_n si intende:  f(t_n, u_n) = M * u_n
    % ma: f_{n} = M * u_{n}   ,  quindi otteniamo: 
    % (I-6/11*h*M) * u_{n}  =  18/11*u_{n-1} - 9/11*u_{n-2} + 2/11*u_{n-3}
    % ottengo quindi un sistema lineare del tipo:  Ax=b, dove x=u_{n}.
    % Ciclo quindi su n, da n=4 fino ad numel(t_vett)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for n=4:numel(t_vett)

        t = t_vett(n-1);    % Qui t=t_{n-1}, lo uso per ricavare 
                            % u_{n}, nuovo nodo

        b = 18/11*u_n_meno_1 - 9/11*u_n_meno_2 + 2/11*u_n_meno_3;
